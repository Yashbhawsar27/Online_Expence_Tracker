<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ page import="javax.servlet.http.HttpSession" %>
<%@ page import="com.entity.User" %>
<%@ page import="com.dao.UserDao" %>

<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Login Page</title>
    <%@ include file="component/all_css.jsp" %>

    <style type="text/css">
        .card-sh {
            box-shadow: 0 0 6px 0 rgba(0, 0, 0, 3);
        }
    </style>
</head>
<body class="bg-light">
    <%@ include file="component/navbar.jsp" %>
    <div class="container p-5">
        <div class="row">
            <div class="col-md-6 offset-md-3">
                <div class="card card-sh">
                    <div class="card-header">
                    <p class="text-center fs-3">Login Page</p>
                        <%-- Check for the "msg" attribute in the session --%>
                        <%
                            HttpSession sessionObj = request.getSession();
                            Object msgAttribute = sessionObj.getAttribute("msg");
                            if (msgAttribute != null && !msgAttribute.toString().isEmpty()) {
                                String msg = msgAttribute.toString();
                        %>
                           
                            <p class="text-center text-danger fs-4"><%=msg%></p>
                            <%
                                // Remove the "msg" attribute from the session after displaying it
                                sessionObj.removeAttribute("msg");
                            }
                            %>
                    </div>
                    <div class="card-body">
                        <form action="login" method="post">
                            <div class="mb-3">
                                <label>Email</label>
                                <input type="email" name="email" class="form-control">
                            </div>
                            <div class="mb-3">
                                <label>Password</label>
                                <input type="password" name="password" class="form-control">
                            </div>
                            <button class="btn btn-dark col-md-12">Login</button>
                            <div class="text-center mt-2">
                                <span>Don't have an account</span>
                                <a href="register.jsp" class="text-decoration-none">create account</a>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>
</html>
